...
function ProfProfile(){
   ...
   useEffect(() => {
       const getProfData = async () => {
         try {
           const response = await fetch(`http://localhost:8080/mark-it/professor/${professorId}`);
   ...
   const handleSaveChange = (e) => {
       ...
       const editedUniversity = e.target[1].value;
       const editedDepartment = e.target[2].value;
       const editedEmail = e.target[3].value;
       const editedPassword = e.target[4].value;
   
       // send the updated data to the server 
       fetch('http://localhost:8080/mark-it/professor/' + professorId + '/updateProfessor', {
       method: 'PUT',
       headers: {
                ...
                <div className="edit">
                    ...
                    { showProfInfo &&
                        <div className="displayInfoForm">
                          <label>{professorData.fullName}</label>
                          <label>{professorData.university}</label>
                          <label>{professorData.department}</label>
                          <label>{professorData.email}</label>
                          {/* <label>{professorData.password}</label> */}
                        </div>
                        }
             
                {  editInfo &&
                   <form className="editInfoForm" onSubmit={e => handleSaveChange(e)}>
                       <input type="text" name="fullName" placeholder="Ονοματεπώνυμο" defaultValue={professorData.fullName} />
                       <input type="text" name="university" placeholder="Πανεπιστήμιο" defaultValue={professorData.university} />
                       <input type="text" name="department" placeholder="Τμήμα"  defaultValue={professorData.department}/>
                       <input type="email" name="email" placeholder="E-mail"  defaultValue={professorData.email} />
                       <input type="password" name="password" className="crtPswd" placeholder="Κωδικός"  defaultValue={professorData.password} />  
                       <span>
